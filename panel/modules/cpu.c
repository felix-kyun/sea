...
load_cpu_info(void)
{
    ...
    if (fscanf(stat, "cpu  %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu", &user, &nice, &system, &idle, &iowait, &irq,
...
get_cpu_total_time(void)
{
    return user + nice + system + idle + iowait + irq + softirq + steal + guest + guest_nice;
}

static inline uint64_t
get_cpu_idle_time(void)
{
    return idle + iowait;
}

static inline void
set_cpu_usage(ModuleState* state, int usage)
{
