...
void
string_array_add(StringArray* array, const char* item)
{
...
void
map_set(LinearMap* map, const char* section, const char* key, const char* value)
{
...
const char*
map_get(LinearMap* map, const char* section, const char* key)
{
    char* full_key = create_key(section, key);
    for (int i = 0; i < map->length; i++) {
        if (strcmp(map->keys->items[i], full_key) == 0) {
...
char*
create_key(const char* section, const char* key)
{
...
void
string_array_from_csv(StringArray* array, const char* csv)
{
...
char*
find_binary(const char* binary_name)
{
...
bool
check_binaries(const char** binaries, size_t count)
{
